...
const transformLabData = (labData: any) => {
  // Primero, asume que labData es un objeto que contiene todas las pruebas
  let results = [];
  for (const [key, value] of Object.entries(labData)) {
    // Evita incluir el ID o cualquier otro valor no deseado
...
export const LabPatientTable = ({ id }: { id: number }) => {
  ...
  const fetchLabsDetail = async () => {
    try {
      ...
      console.log(transformedData); // Verifica que cada elemento sea { testName: string, value: string }
